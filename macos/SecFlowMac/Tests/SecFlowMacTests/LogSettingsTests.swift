...
final class LogSettingsTests: XCTestCase {
    @MainActor
    func testLogSettingsRendersAtMinimumWindowSize() throws {
        let size = NSSize(
            width: SettingsWindowMetrics.defaultSize.width,
            height: SettingsWindowMetrics.minSize.height
        )
        let hostingView = NSHostingView(
            rootView: SettingsView(initialSection: .logs, loadsData: false)
        ...
        if let snapshotPath = ProcessInfo.processInfo.environment["SECFLOW_LOG_SETTINGS_SNAPSHOT"] {
