...
TRIAL_CONFIG="$TAURI_DIR/src-tauri/tauri.trial14.conf.json"
TRIAL_IDENTIFIER="ai.secflow.security-agent.trial14days"
TRIAL_PRODUCT_NAME="神盾 14 天试用版"
TARGET_TRIPLE="${SECFLOW_TAURI_TARGET:-aarch64-apple-darwin}"

SECFLOW_TAURI_TRIAL_BUILD=1 \
SECFLOW_TAURI_BACKEND_PORT=18784 \
SECFLOW_TRIAL_DURATION_HOURS=336 \
SECFLOW_APP_RELEASE_CHANNEL="14天试用版" \
SECFLOW_TRIAL_KEYCHAIN_SERVICE="$TRIAL_IDENTIFIER" \
SECFLOW_REQUIRE_CLEAN_SOURCE=1 \
SECFLOW_TAURI_CONFIG="$TRIAL_CONFIG" \
SECFLOW_TAURI_BUILD_ROOT="${SECFLOW_TAURI_TRIAL_BUILD_ROOT:-${TMPDIR:-/tmp}/aegisal-tauri-macos-trial14-build}" \
...
[ -f "$DMG_PATH" ] || { echo "Missing trial DMG in: $DMG_DIR" >&2; exit 1; }
