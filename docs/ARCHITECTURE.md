...
漏洞情报采集阶段会对需要中文展示的漏洞字段进行本地批量翻译和确定性风险等级本地化，并将译文与原文一起写入 catalog。该翻译使用随应用打包的离线 Translation MCP，不依赖用户模型配置，不消耗模型 Token。资讯中心不进行翻译，始终展示资讯源提供的原始标题和摘要。报告生成阶段禁止重新遍历大结果调用翻译服务，从而避免重复计算和报告卡顿。
