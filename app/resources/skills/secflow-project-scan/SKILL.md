...
1. For a user-authorized project scan, dispatch every selected language node to `SecFlow Code Scan MCP / scan_language` over the managed loopback SSE transport. Project-license identification is delegated to the SBOM Agent and its independent `SecFlow License MCP`; the Code Scan MCP does not expose a license tool.
